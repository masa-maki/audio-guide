...
const Home: NextPage = () => {
  ...
  const formatTimeRemains = (time: number, duration: number) => {
  ...
  return (
    ...
    {/*<
      ...
      <main className="flex items-center justify-center h-screen min-h-dvh h-dvh flex-col p-0 text-white">
        <div className="flex h-full h-dvh sm:max-h-[812px] w-full sm:max-w-[375px] sm:rounded-lg text-center shadow bg-[#2B1A36]">
      ...
      {/* <main className="flex items-center justify-center h-screen h-full w-full flex-col p-0 text-white">
        <div className="w-full h-full flex shadow bg-[#2B1A36]">
          <div className="w-full h-full flex flex-col space-y-0">*/}
            ...
            <section id="image" className="w-full basis-8/12 overflow-hidden rounded-b-3xl relative">
            {/*<section id="image" className="w-full basis-7/12 h-20 items-stretch text-black bg-[#FFFFFF]">*/}
              <Image
                ...
                alt={guide.title}
              />
              {/* <p className="absolute mt-2 mx-4 top-0 left-0 drop-shadow">&lsaquo; Back to Audio guide top</p> */}
            </section>
            ...
            {/* <section id="information" className="flex w-full items-stretch flex-1">*/}
              ...
              <div>
                ...
                <div className="flex mx-4 text-sm justify-between">
                  <p>{formatTime(currentTime)}</p>
                  <p>{formatTimeRemains(currentTime, duration)}</p>
                </div>
              </div>
              <div id="player" className="flex flex-row gap-x-5 items-center justify-center">
                <button 
                  onClick={ () => skipTime(-10) } 
                  className="rounded-full p-2 text-white shadow-s"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 14 14" fill="none">
                ...
                <button
                  ...
                  type="button"
                  className="bg-white rounded-full p-3 text-white shadow-s"
                >
                  {!isPlaying ? (
                    <PlayIcon className="h-8 w-8 text-[#2B1A36]" aria-hidden="true" />
                  ) : (
                    <PauseIcon className="h-8 w-8 text-[#2B1A36]" aria-hidden="true" />
                  )}
                </button>
                <button 
                  onClick={ () => skipTime(10) } 
